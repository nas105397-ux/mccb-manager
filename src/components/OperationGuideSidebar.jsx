...
const GUIDE_CONTENT = {
  operation: {
    ...
    title: "禁止札操作",
    notice: "操作前に対象設備名、状態、電気室、子札の貸出状況を必ず確認してください。",
    sections: [
      {
        title: "対象を確認",
        items: [
          "カード上部の色帯は実際の電源状態を示します。緑は送電中系（送電中・依頼発行中）、赤は停電中系（停電中・送電可能）です。",
          "送電可能（子札返却済み）や依頼発行中（停電手配中）などの詳しい状況は、状態ラベルの色と文言で確認します。",
      ...
      },
      {
        ...
        items: [
          ...
          "「表示・並び順」を開くと、表示する電気室を複数選択できます。未選択の場合は全電気室を表示します。",
          "カードは状態、電気室、お気に入り、登録順の優先順で表示されます。状態の優先順位は「表示・並び順」で上下ボタンを使って変更できます。",
      ...
      },
      {
        ...
        items: [
          ...
          "依頼発行中の設備を停電にすると、カードは停電中として再分類され、一覧の停電中の優先位置へ移動します。",
