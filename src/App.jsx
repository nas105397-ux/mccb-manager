...
function AppContent() {
  ...
  if (controller.activeTab === '/monitor') {
    return (
      <DashboardView onClose={() => controller.goTo('/')} />
    );
  }
      ...
      <div className="max-w-7xl mx-auto space-y-4 print:space-y-0">
        ...
        <div className="flex justify-between items-center bg-white p-2 rounded-xl border border-gray-200 shadow-sm print:hidden">
          <div className="flex flex-wrap gap-2">
            ...
            {controller.navItems.map((item) => (
              <button
                ...
                className={`px-4 py-2 rounded-lg text-xs font-black transition-all cursor-pointer ${
                  controller.activeTab === item.path ? 'bg-blue-600 text-white shadow-md' : 'text-gray-500 hover:bg-gray-100'
                }`}
              >
                {item.label}
              </button>
            ...
            <button onClick={() => controller.goTo('/monitor')} className="px-4 py-2 rounded-lg text-xs font-black text-teal-600 border border-teal-200 bg-teal-50 hover:bg-teal-100 transition-all cursor-pointer shadow-sm flex items-center gap-1">🖥️ 電気室モニター</button>
          </div>
          
          <button onClick={controller.handleToggleAdmin} className={`px-3 py-1.5 rounded-lg text-[11px] font-black tracking-wide border cursor-pointer shadow-sm transition-all whitespace-nowrap mr-1 ${ controller.isAdmin ? 'bg-amber-50 border-amber-300 text-amber-700 hover:bg-amber-100' : 'bg-gray-50 border-gray-300 text-gray-500 hover:bg-gray-100' }`}>
            {controller.isAdmin ? '🔓 モード: 管理者' : '🔒 モード: 一般ユーザー'}
          </button>
        </div>
        ...
        <Routes>
          <Route path="/" element={
            <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mt-4">
                {controller.filteredMccbList.map((mccb) => (
                  <MccbCard
                    key={mccb.id}
                    mccb={mccb}
                    borrowedCount={controller.borrowedCountMap[mccb.id] ?? 0}
              ...
              {controller.currentMccb && (
                <ControlModal
                  ...
                  borrowedCount={controller.borrowedCountMap[controller.currentMccb.id] ?? 0}
          ...
          <Route
            path="/request"
            element={<RequestFormPanel mccbList={controller.mccbList} onAddRequest={controller.addRequest} requests={controller.requests} deviceGroups={controller.deviceGroups} />}
          />
          <Route
            path="/request-list"
            element={<RequestListPanel requests={controller.requests} requestHistory={controller.requestHistory} mccbList={controller.mccbList} onDeleteRequest={controller.deleteRequest} />}
          />
