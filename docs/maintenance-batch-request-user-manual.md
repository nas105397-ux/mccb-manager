# 修繕日・大修繕日 仮発行手順書（一般作業者向け）

対象画面: `http://192.168.40.59:5000/#/`

> この手順書は、修繕日・大修繕日に予定される停電作業の依頼を、事前に仮発行として登録し、当日に一致する依頼だけを正式発行する手順です。
...
- 仮発行の段階では、子札は割り当てられません。子札は、当日に **発行** した時点の空き札で割り当てられます。
...
- 修繕日・大修繕日で複数の仮発行があっても、依頼ごとに照合してから発行します。
- 複数の依頼をまとめて確認しても、発行は一致した依頼ごとに行います。
